...
print("ecco")
rec_df.to_csv(r'frame_s10_gamma_0_99/rec.csv', sep='\t', encoding='utf-8', header=None, index=False)
input("solo_rec")
...
train_df.to_csv(r'ml-1m/train.csv', sep=',')
test_df.to_csv(r'ml-1m/test.csv', sep=',')

input("continua con Rated_train")

print(train_df['rating'], train_df["rating"])

Rated = Rated_Items(train)

a_file = open("ml-1m/Rated_train.pkl", "wb")
